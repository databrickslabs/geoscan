...
@Skip
// THIS IS NOT A TEST, BUT GENERATES DATA TO VALIDATE ON DEMAND
class PrepData extends AnyFunSuite {
  ...
  test("Generating geo points in multiple clusters") {
    ...
    val pw = new PrintWriter(new FileOutputStream(new File("target/nyc_shapes.csv")))
